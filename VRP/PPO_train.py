...
class TrainPPO:
    ...
    def run_train(self, data_loader, batch_size, valid_loader):
        memory = Memory()
        self.agent.old_policy.to(device)
        # initWeights(self.agent.old_policy)
        ...
        for i in range(self.epoch):
            ...
            for batch_idx, batch in enumerate(data_loader):
                ...
                actions, log_p = self.agent.old_policy.act(
            ...
            cost = rollout(self.agent.policy, valid_loader, batch_size, self.steps)
            cost = cost.mean()
            ...
            torch.save(self.agent.old_policy.state_dict(), save_path)
...
def train():
    ...
    for (
        ...
        data_loader = create_data(n_nodes, data_size, batch_size)
        valid_loader = create_data(n_nodes, valid_size, batch_size)
        ...
        trainppo.run_train(data_loader, batch_size, valid_loader)
